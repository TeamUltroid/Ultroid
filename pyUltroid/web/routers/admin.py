...
async def update_bot(fast: bool = False) -> dict:
    ...
    try:
        ...
        if fast:
            stdout, stderr, code = await bash("git pull -f && pip3 install -r requirements.txt")
            if code != 0:
                raise Exception(f"Fast update failed: {stderr}")
            return {"status": "success", "message": "Fast update completed", "restart_required": True}
            
        # Check for updates
        origin = repo.remotes.origin
        origin.fetch()
        if not repo.is_dirty():
            commits_behind = sum(1 for _ in repo.iter_commits(f'{branch.name}..origin/{branch.name}'))
            if commits_behind == 0:
                return {"status": "success", "message": "Already up to date!", "update_available": False}
        
        # Pull changes
        stdout, stderr, code = await bash("git pull && pip3 install -r requirements.txt")
        if code != 0:
...
# Route handlers
@route("/api/admin/update", method="POST", owner_only=True, description="Update the bot from repository")
...
@route("/api/admin/restart", method="POST", owner_only=True, description="Restart the bot")
