...
async def proxy_request(request: web.Request, path: str, method: str = "GET", require_auth: bool = False):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if method == "GET":
                ...
                if request.content_type.startswith('multipart/'):
                    ...
                    for key, value in data.items():
                        if hasattr(value, 'file'):
                            form_data.add_field(key, value.file, 
                    ...
                    async with session.post(target_url, headers=headers_without_content_type, data=form_data) as response:
                        try:
                            resp_json = await response.json()
                            return web.json_response(resp_json, status=response.status)
                        except:
                    ...
                    async with session.post(target_url, headers=headers, json=body) as response:
                        resp_json = await response.json()
                        return web.json_response(resp_json, status=response.status)
                ...
                if request.content_type.startswith('multipart/'):
                    ...
                    async with session.put(target_url, headers=headers_without_content_type, data=form_data) as response:
                        try:
                            resp_json = await response.json()
                            return web.json_response(resp_json, status=response.status)
                        except:
                    ...
                    body = await request.json()
                    async with session.put(target_url, headers=headers, json=body) as response:
                        resp_json = await response.json()
                        return web.json_response(resp_json, status=response.status)
...
async def proxy_plugin_get(request: web.Request):
    ...
    return await proxy_request(request, f"/api/v1/plugins/{plugin_id}", require_auth=True)
...
async def proxy_plugin_update(request: web.Request):
    ...
    plugin_id = request.match_info["plugin_id"]
    return await proxy_request(request, f"/api/v1/plugins/{plugin_id}", "PUT", require_auth=True)

async def proxy_plugin_delete(request: web.Request):
    """Proxy DELETE /api/v1/plugins/{plugin_id}"""
    plugin_id = request.match_info["plugin_id"]
    return await proxy_request(request, f"/api/v1/plugins/{plugin_id}", "DELETE", require_auth=True)
...
async def proxy_plugins_by_uploader(request: web.Request):
    ...
    return await proxy_request(request, f"/api/v1/plugins/uploader/{uploader_id}", require_auth=True)
    
async def proxy_plugins_compute_diff(request: web.Request):
    """Proxy POST /api/v1/plugins/compute_diff"""
    return await proxy_request(request, "/api/v1/plugins/compute_diff", "POST", require_auth=True)

def setup_plugin_routes(app):
    """Setup routes for plugins API"""
    # Public routes - no auth required
    app.router.add_get("/api/v1/plugins", proxy_plugins_list)
    app.router.add_get("/api/v1/plugins/uploader/{uploader_id}", proxy_plugins_by_uploader)
