...
async def kang_cmd(ult):
    ...
    if dl:
        ...
        await ult.client(UploadMediaRequest(types.InputPeerSelf(), upl))
    ...
    # ait ult.client(CreateStickerSetRequest(user_id=sender.id, title=title,
    # short_name=sn, stickers=[SetItem(file)], animated=animated))
