...
async def rmbgapi(event):
    ...
    async with event.client.conversation(pru) as conv:
        ...
        if themssg == "/cancel":
            return await conv.send_message("Cancelled!!", buttons=get_back_button("apiset"))
        else:
            await setit(event, var, themssg)
            await conv.send_message("{} changed to {}".format(name, themssg), buttons=get_back_button("apiset"))
