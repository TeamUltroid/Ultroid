...
async def gitsearch(event):
    usrname = event.pattern_match.group(1).strip().strip()
    if not usrname:
...
async def google(event):
    inp = event.pattern_match.group(1).strip().strip()
    if not inp:
...
async def goimg(event):
    query = event.pattern_match.group(1).strip().strip()
    if not query:
...
async def siesace(e):
    song = e.pattern_match.group(1).strip().strip()
    if not song:
