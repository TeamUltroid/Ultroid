	...
	if Redis("GDRIVE_CLIENT_ID") == None or Redis("GDRIVE_CLIENT_SECRET") == None:
		...
		async with asst.conversation(ultroid_bot.uid) as conv:
			...
			try:
				...
				return await repl.reply("`Something went wrong! Send `/auth` again.\nIf same happens contact `@TheUltroid")
	else:
...
async def _(event):
	...
	if event.reply_to_msg_id and not input_str:
		...
		if os.path.exists(input_str):
			...
			return await eod(mone, "File Not found in local server. Give me a file path :((", time=5)
	if required_file_name:
		http = authorize(TOKEN_FILE, None)
		file_name, mime_type = file_ops(required_file_name)
		try:
			g_drive_link = await upload_file(
			http, required_file_name, file_name, mime_type, mone, Redis("GDRIVE_FOLDER_ID")
			)
		...
		return await eod(
		mone, "`File Not found in local server.`", time=10
		)
...
async def sch(event):
	...
	if Redis("GDRIVE_FOLDER_ID") is not None:
		query = "'{}' in parents and (title contains '{}')".format(Redis("GDRIVE_FOLDER_ID"), input_str)
	else:
...
async def _(event):
	...
	if os.path.isdir(input_str):
		...
		dir_id = await create_directory(http, os.path.basename(os.path.abspath(input_str)), Redis("GDRIVE_FOLDER_ID"))
		await DoTeskWithDir(http, input_str, event, dir_id)
		dir_link = "https://drive.google.com/folderview?id={}".format(dir_id)
		await eod(a, f"**Successfully Uploaded Folder To G-Drive...**\n[{input_str}]({dir_link})")
	else:
...
async def _(event):
    if Redis("GDRIVE_FOLDER_ID"):
    	folder_link = "https://drive.google.com/folderview?id=" + Redis("GDRIVE_FOLDER_ID")
