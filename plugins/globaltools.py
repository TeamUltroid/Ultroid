	...
	async for ggban in e.client.iter_dialogs():
		if ggban.is_group or ggban.is_channel:
			try:
				...
				pass
	ungban(userid)
	await xx.edit(f"`Ungbanned` [{name}](tg://user?id={userid}) `in {chats} chats.\nRemoved from gbanwatch.`")
...
async def _(e):
	...
	if is_gbanned(userid):
		return await eod(xx, "`User is already gbanned and added to gbanwatch.`", time=4)
	async for ggban in e.client.iter_dialogs():
		if ggban.is_group or ggban.is_channel:
			try:
				...
				pass
	gban(userid)
	await xx.edit(f"`Gbanned` [{name}](tg://user?id={userid}) `in {chats} chats.\nAdded to gbanwatch.`")
	...
	if e.user_joined or e.added_by:
		...
		if is_gbanned(str(user.id)):
			if chat.admin_rights.ban_users:
				try:
					await e.client.edit_permissions(chat.id, user.id, view_messages=False)
					gban_watch = f"`Gbanned User` [{user.first_name}](tg://user?id={user.id}) `Spotted\n"
					gban_watch += f"Banned Successfully`"
				except:
					pass
			else:
				gban_watch = f"@admins `Gbanned user` [{user.first_name}]({user.id}) `Spotted.`"
