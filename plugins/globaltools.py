...
async def _(e):
    ...
    reason = ""
    if e.pattern_match.group(1).strip().strip():
...
async def gucast(event):
    ...
    xx = event.pattern_match.group(1).strip().strip()
    if xx:
        msg, btn = get_msg_button(event.text.split(maxsplit=1)[1])
    elif event.is_reply:
        reply = await event.get_reply_message()
...
async def gkick(e):
    ...
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
    elif e.pattern_match.group(1).strip().strip():
        userid = await e.client.parse_id(e.pattern_match.group(1).strip().strip())
    elif e.is_private:
        userid = e.chat_id
    else:
