...
@Client.on_message(filters.command("stopvc", HNDLR) & filters.user(AUTH) & ~filters.edited)
...
@Client.on_message(filters.command("resumevc", HNDLR) & filters.user(AUTH) & ~filters.edited)
async def ustop(_, message):
  await stopvc(_, message)